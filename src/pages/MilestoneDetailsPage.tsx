...
import { FiPaperclip, FiPlus, FiTrash2, FiDollarSign, FiUsers, FiClock, FiCheckCircle } from 'react-icons/fi';
...
const milestoneData: MilestoneDetails = {
    ...
    files: [
        { 
            id: 1, name: 'starlight_vocal_take1.wav', url: '#',
            comments: [
                { id: 1, user: { name: 'Luna', avatarUrl: 'https://i.pravatar.cc/150?img=2' }, timestamp: '0:15', text: 'Chỗ này em hát hơi chênh một chút, mình thu lại nhé!' },
                { id: 2, user: { name: 'Alex Thorne', avatarUrl: 'https://i.pravatar.cc/150?img=1' }, timestamp: '0:32', text: 'Đoạn này feeling rất tốt, giữ nguyên nhé.' },
            ]
        },
        ...
        <div className="relative mb-4 cursor-pointer">
            ...
            {file.comments.map(comment => (
                <img 
                    ...
                    style={{ left: `${parseInt(comment.timestamp.split(':')[1]) / 60 * 100}%` }} // Simplified positioning
                />
            ))}
        </div>
                ...
                <div key={comment.id} className="flex items-start space-x-3 text-sm">
                    ...
                    <div>
                        <p>
                            ...
                            <span className="text-accent ml-2 text-xs font-mono">{comment.timestamp}</span>
                        </p>
...
function MilestoneDetailsPage() {
    ...
    const totalSplitAmount = milestone.splits.reduce((sum, s) => sum + s.amount, 0);
            ...
            <div className="max-w-7xl mx-auto">
                <header className="mb-8">
                    <p className="text-sm text-text-secondary">
                        Dự án: <a href="#" className="text-accent hover:underline">{milestone.projectName}</a>
                    </p>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-2">
                        ...
                        <div className="flex items-center space-x-4 mt-3 sm:mt-0">
                            ...
                            <select defaultValue={milestone.status} className="bg-dark-surface border border-border-color rounded-lg px-3 py-1.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent">
                    ...
                    <main className="lg:col-span-2 space-y-8">
                        <section className="bg-dark-surface p-6 rounded-xl border border-border-color">
                             <div className="flex justify-between items-center mb-4">
                                <h2 className="text-xl font-bold text-white">Files & Phản hồi</h2>
                                <button className="text-sm bg-accent text-white font-semibold py-1.5 px-3 rounded-md hover:bg-opacity-80 transition-colors flex items-center space-x-2">
                                   <FiPlus size={16}/><span>Tải lên File</span>
                                </button>
                            </div>
                            <div className="space-y-6">
                                {milestone.files.map(file => <AudioPlayer key={file.id} file={file} />)}
                            </div>
                        </section>
                    </main>
                    ...
                    <aside className="lg:col-span-1 space-y-8">
                        <section className="bg-dark-surface p-6 rounded-xl border border-border-color">
                             ...
                             <div className="space-y-3 text-sm">
                                <div className="flex justify-between items-center"><span className="text-text-secondary">Ngân sách</span><span className="font-bold text-white">{new Intl.NumberFormat('vi-VN').format(milestone.cost)} VNĐ</span></div>
                                <div className="flex justify-between items-center"><span className="text-text-secondary">Đã phân chia</span><span className="font-bold text-white">{new Intl.NumberFormat('vi-VN').format(totalSplitAmount)} VNĐ</span></div>
                                <div className="flex justify-between items-center pt-2 border-t border-border-color"><span className="text-text-secondary">Còn lại</span><span className="font-bold text-yellow-400">{new Intl.NumberFormat('vi-VN').format(milestone.cost - totalSplitAmount)} VNĐ</span></div>
                             </div>
                        ...
                        <section className="bg-dark-surface p-6 rounded-xl border border-border-color">
                            <div className="flex justify-between items-center mb-4">
                                <h2 className="text-xl font-bold text-white">Thành viên & Phân chia</h2>
                                <button className="text-sm text-accent hover:underline flex items-center space-x-1"><FiPlus size={14}/><span>Quản lý</span></button>
                            </div>
                             <div className="space-y-3">
                                {milestone.splits.map(split => {
                                    const member = milestone.members.find(m => m.id === split.memberId);
                                    if (!member) return null;
                                    return (
                                        <div key={split.memberId} className="flex items-center justify-between bg-dark-bg p-3 rounded-lg">
                                            ...
                                            <p className="font-bold text-accent text-sm">{new Intl.NumberFormat('vi-VN').format(split.amount)}</p>
                                        </div>
                                    )
                                })}
                             </div>
