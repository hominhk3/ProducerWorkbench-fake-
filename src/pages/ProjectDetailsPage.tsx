...
import { FiPlus, FiMoreVertical, FiPaperclip, FiMessageSquare, FiFileText, FiUsers, FiDollarSign } from 'react-icons/fi';
import { FaStar, FaRegStar } from 'react-icons/fa';
import type { Project, Member, Milestone, Contract } from '../types';
...
function ProjectDetailsPage() {
  ...
  const totalFee = projectData.milestones.reduce((sum, m) => sum + m.cost, 0);
  const balance = projectData.transactions.reduce((sum, t) => sum + t.amount, 0);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="bg-dark-surface p-6 rounded-2xl border border-border-color mb-8">
            <div className="flex flex-col sm:flex-row items-start gap-6">
                <img src={projectData.imageUrl} alt={projectData.name} className="w-full sm:w-32 h-32 object-cover rounded-lg"/>
                <div className="w-full">
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <p className="text-accent font-medium mt-1">Khách hàng: {projectData.client}</p>
                        </div>
                    </div>
                    <div className="mt-4">
                        ...
                        <div className="w-full bg-dark-bg rounded-full h-2.5">
                            <div className="bg-accent h-2.5 rounded-full" style={{width: `${projectData.progress}%`}}></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <aside className="lg:col-span-1 space-y-8">
                <section className="bg-dark-surface p-6 rounded-xl border border-border-color">
                     ...
                     {projectData.contract ? (
                        <div>
                             <p className="text-sm text-text-secondary">Trạng thái: <span className="font-medium text-green-400">{projectData.contract.status}</span></p>
                             <button className="w-full mt-4 bg-dark-bg border border-border-color text-text-primary font-semibold py-2 rounded-lg hover:border-accent transition-colors">Xem chi tiết</button>
                        </div>
                     ) : (
                        <div className="text-center">
                             <FiFileText size={32} className="mx-auto text-text-secondary mb-2"/>
                             <p className="text-sm text-text-secondary mb-4">Chưa có hợp đồng cho dự án này.</p>
                             <button className="w-full bg-accent text-white font-bold py-2 rounded-lg hover:bg-opacity-80 transition-colors">Tạo Hợp đồng</button>
                        </div>
                     )}
                </section>
                ...
                <section className="bg-dark-surface p-6 rounded-xl border border-border-color">
                    ...
                    <div className="mt-6 pt-4 border-t border-border-color">
                        <h3 className="font-bold text-white text-md mb-4">Phân chia Doanh thu (Split Sheet)</h3>
                            ...
                            {projectData.splits.map(split => {
                                ...
                                return (
                                    <div key={split.memberId} className="flex items-center justify-between text-sm">
                                        ...
                                        <p className="font-semibold text-accent">{split.percentage}%</p>
                                    </div>
                                )
                            })}
                        </div>
