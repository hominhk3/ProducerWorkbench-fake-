      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center space-x-4 mt-4 sm:mt-0">
            <div className="relative">
              ...
              <input
                ...
                className="bg-dark-surface border border-border-color rounded-lg pl-10 pr-4 py-2 w-full sm:w-64 focus:ring-2 focus:ring-accent focus:outline-none"
              />
            </div>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          {dashboardData.stats.map((stat) => (
            <div
              key={stat.id}
              className="bg-dark-surface p-6 rounded-xl border border-border-color"
            >
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 text-sm font-medium rounded-full whitespace-nowrap transition-colors ${
                      filter === f
                        ...
                        : "bg-dark-surface text-text-secondary hover:bg-border-color"
                    }`}
                  >
                    {f}
                  </button>
            ...
            <div className="bg-dark-surface rounded-xl border border-border-color">
              <div className="grid grid-cols-1 divide-y divide-border-color">
                {filteredProjects.map((project) => (
                  <div
                    key={project.id}
                    className="p-6 hover:bg-border-color/20 transition-colors"
                  >
                    <Link to={"/milestone"}>
                ...
                {dashboardData.tasks.map((task) => (
                  <div key={task.id}>
                    ...
                    <p className="text-xs text-accent">
                      {task.project} &middot;{" "}
                      <span className="text-text-secondary">{task.due}</span>
                    </p>
                  </div>
                ))}
              </div>
