...
import { FiChevronRight, FiChevronLeft, FiUser, FiBriefcase, FiUploadCloud } from 'react-icons/fi';
...
function CreateProjectPage() {
  ...
  const handleScopeSelectAndSubmit = (scope: 'personal' | 'client') => {
    ...
    alert(`Dự án đã được tạo! Chuyển hướng đến trang chi tiết ${scope === 'personal' ? 'Dự án Cá nhân' : 'Dự án Hợp tác'}...`);
  };
        ...
        {steps.map((label, index) => (
          <React.Fragment key={label}>
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold transition-all ${currentStep > index + 1 ? 'bg-green-500 text-white' : currentStep === index + 1 ? 'bg-accent text-white' : 'bg-dark-surface border-2 border-border-color text-text-secondary'}`}>
                {index + 1}
              </div>
              <p className={`ml-3 font-medium ${currentStep === index + 1 ? 'text-white' : 'text-text-secondary'}`}>{label}</p>
            </div>
                    ...
                    <div className="mt-2 flex justify-center rounded-lg border border-dashed border-border-color px-6 py-10 bg-dark-bg">
                      <div className="text-center">
                        ...
                        <div className="mt-4 flex text-sm justify-center">
                          <label htmlFor="file-upload" className="relative cursor-pointer rounded-md font-semibold text-accent">
      ...
      case 2:
        return (
            <div className="max-w-2xl mx-auto text-center">
                <h2 className="text-3xl font-bold text-white mb-4">Hoàn tất & Lưu trữ</h2>
                <p className="text-text-secondary mb-8">Chọn loại hình để lưu trữ và quản lý dự án này.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <button onClick={() => handleScopeSelectAndSubmit('personal')} className="bg-dark-surface p-8 rounded-2xl border-2 border-border-color hover:border-accent hover:bg-accent/10 transition-all text-left">
                        <FiUser size={32} className="text-accent mb-4"/>
                        <h3 className="text-xl font-bold text-white mb-2">Lưu thành Dự án Cá nhân</h3>
                        <p className="text-text-secondary">Dự án sẽ được quản lý trong không gian cá nhân của bạn.</p>
                    </button>
                     <button onClick={() => handleScopeSelectAndSubmit('client')} className="bg-dark-surface p-8 rounded-2xl border-2 border-border-color hover:border-accent hover:bg-accent/10 transition-all text-left">
                        <FiBriefcase size={32} className="text-accent mb-4"/>
                        <h3 className="text-xl font-bold text-white mb-2">Lưu thành Dự án Hợp tác</h3>
                        <p className="text-text-secondary">Dự án sẽ có đầy đủ tính năng hợp đồng, cột mốc, ví tiền...</p>
                    </button>
                </div>
            </div>
        );
    ...
    <div className="bg-dark-bg min-h-screen font-inter text-text-primary p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between mt-8">
          <button onClick={prevStep} disabled={currentStep === 1} className="bg-border-color text-text-primary font-bold py-3 px-6 rounded-lg flex items-center disabled:opacity-50">
            <FiChevronLeft />
            <span>Quay lại</span>
          </button>
          {currentStep === 1 && (
            <button onClick={nextStep} className="bg-accent text-white font-bold py-3 px-6 rounded-lg flex items-center">
                <span>Tiếp tục</span>
                <FiChevronRight />
            </button>
          )}
